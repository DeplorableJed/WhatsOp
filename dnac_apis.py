...
def get_all_device_info(dnac_jwt_token):
    ...
    print(all_device_response.status_code)
...
def get_site_health(epoch_time, dnac_jwt_token):
    """
    This function will call the site health intent APIs and return the health for all sites
    :param epoch_time: epoch time including msec
    :param dnac_jwt_token: DNA C token
    :return: site health
    """
    url = DNAC_URL + '/dna/intent/api/v1/site-health?timestamp=' + str(epoch_time)
    header = {'content-type': 'application/json', 'x-auth-token': dnac_jwt_token}
    response = requests.get(url, headers=header, verify=False)
